...
def clean_and_anonymise_text(text):
    ...
    text = re.sub(r"\b(mr|mrs|ms|dr|prof)\.?\s+[a-z]+\b", "<NAME>", text)
...
for col in ["sender", "receiver"]:
    if col in df_clean.columns:
        df_clean[col] = df_clean[col].fillna("unknown@unknown.com")
...
df_clean.isna().sum()
df_clean.to_csv("../data/processed/Nazario_cleanedData22.csv", index=False)
